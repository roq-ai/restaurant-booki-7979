...
function DashboardPage() {
  ...
  const restaurantStat = useSWR<PaginatedInterface<RestaurantInterface>>(
    () => `/restaurants`,
    () => getRestaurants({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Restaurants',
      result: restaurantStat?.data?.totalCount,
      isLoading: restaurantStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
